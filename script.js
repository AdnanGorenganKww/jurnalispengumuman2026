/* ==========================================================================
   PORTAL SELEKSI JURNALISTIK — script.js
   Berisi seluruh logika: database peserta, navigasi layar, validasi,
   animasi verifikasi, hasil seleksi, digital result card (3D tilt),
   confetti, dan particle background.
   ========================================================================== */
...
(() => {
  ...
  /* ========================================================================
     1. DATABASE PESERTA
     Tambahkan / kurangi objek di array ini sesuai jumlah peserta asli.
     Tidak perlu mengubah HTML maupun CSS sama sekali.
     ======================================================================== */
  const peserta = [
  ...
  /** Pindah ke satu layar, sembunyikan semua layar lain. */
  function showScreen(key) {
    Object.values(screens).forEach((el) => el.classList.remove('is-active'));
    const target = screens[key];
    // paksa reflow supaya animasi CSS screen (fade-in) selalu terpicu ulang
  ...
  /**
   * Memutar ulang semua animasi CSS (keyframe) di dalam sebuah elemen.
   * Dipakai supaya animasi "muncul satu per satu" pada layar hasil
   * dan opening bisa berjalan lagi setiap kali layar itu ditampilkan.
   */
  ...
  /* ========================================================================
     3. PARTICLE BACKGROUND (floating particles)
     ======================================================================== */
  ...
  function runOpeningSequence() {
    ...
    // animasikan progress bar 0% -> 100% selaras dengan durasi opening di CSS
  ...
  $('#btn-welcome-home').addEventListener('click', () => {
    showScreen('home');
  });
  ...
  $('#form-username').addEventListener('submit', (e) => {
    e.preventDefault();
    const raw = $('#input-username').value.trim().toUpperCase();
  ...
  /* ========================================================================
     ...
     ======================================================================== */
  const CIRCUMFERENCE = 2 * Math.PI * 42; // sesuai r=42 pada SVG spinner

  function resetLoadingScreen() {
    $$('.loading-step').forEach((step) => {
      step.classList.remove('is-active', 'is-done');
    });
    $('#loading-percent').textContent = '0%';
    $('#loading-bar-fill').style.width = '0%';
    $('.loading-spinner-arc').style.strokeDashoffset = String(CIRCUMFERENCE);
    $('#loading-status').textContent = 'Mohon tunggu sebentar...';
  }
  ...
  function setProgress(percent) {
    ...
    $('.loading-spinner-arc').style.strokeDashoffset = String(offset);
  }
  ...
  function startVerification(pesertaData) {
    ...
    steps.forEach((step, index) => {
      setTimeout(() => {
        // tandai step sebelumnya selesai
        if (index > 0) {
        ...
        if (index === steps.length - 1) {
          setTimeout(() => {
            ...
            $('#loading-status').textContent = 'VERIFICATION COMPLETE';
          }, stepDelay - 150);
        }
      }, index * stepDelay);
    });
  ...
  function revealResult(data) {
    if (data.status === 'lulus') {
      $('#pass-nama').textContent = data.nama;
      $('#pass-username').textContent = data.username;
      $('#pass-divisi').textContent = data.divisi;
      ...
      $('#fail-nama').textContent = data.nama;
      $('#fail-username').textContent = data.username;
